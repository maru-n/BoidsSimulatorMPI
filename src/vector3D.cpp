...
float operator*(const Vector3D& u,const Vector3D& v){	//内積 vector*vector
	return u.x * v.x + u.y * v.y + u.z * v.z;
}

Vector3D operator*(const Vector3D& v, float k){	//vector*scalar
	Vector3D w;
	w.x = v.x * k;
	w.y = v.y * k;
	w.z = v.z * k;
...
Vector3D operator*(float k ,const Vector3D& v){	//scalar*vector
	Vector3D w;
	w.x = v.x * k;
	w.y = v.y * k;
	w.z = v.z * k;
