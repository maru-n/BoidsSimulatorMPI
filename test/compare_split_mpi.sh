...
mpiexec --hostfile ../../hostfile -n 8 ./boidsim_mpi -s ${setting_file} -T $t_first -o ${data_name_split_first} --velocity-output ${data_name_split_first_v} > /dev/null || exit -1
...
mpiexec --hostfile ../../hostfile -n 8 ./boidsim_mpi -s ${setting_file} -T $t_last -o ${data_name_split_second} --initialization ${data_name_secong_init} > /dev/null || exit -1
